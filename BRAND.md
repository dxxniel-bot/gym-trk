...
| 2026-09-24 | **Laboratorio de macros** (v276): mientras elige, el panel abierto muestra **versiones** de las barras de progreso y del reparto de los macros en un **carrusel** que se desliza izquierda-derecha, con pestañas de texto abajo (`aros · barras · medidor · reparto · tabla`; `aros` = el radar y los anillos de antes), y recuerda la última que vio. La **dona** existe solo en el laboratorio y entra al carrusel solo si la elige: el anillo de kcal sigue siendo la gráfica circular de la app (§4). **Compartir el panel:** `[share]` en macros pregunta `el día · tus comidas` (la tarjeta de siempre) o `el panel de macros` (anillo de kcal + la versión elegida, vertical y a su altura natural) | su encargo del 24-sep: propuestas de barras de progreso y de distribución de los macros, verlas "en un carrusel" dentro del panel (izquierda-derecha) mientras elige, y una versión para compartir |
| 2026-09-24 | **Alta paso a paso** (v277): crear cuenta deja de ser un formulario de una pantalla y pasa a **una pregunta por pantalla**, con `[‹ atrás]` siempre, `//SETUP 3/10` y su barra `[███░░░░░░░]`, en este orden: nombre y gym · **unidades antes del cuerpo** (peso corporal y pesas del gym: el peso se escribe en tu unidad) · cuerpo (con tu mantenimiento en vivo) · actividad · objetivo · cómo entrenas · rutina · metas del día · Salud · listo (resumen + `▶ ir al gym`). Cómo entrenas, rutina, metas y Salud son **para ahora o para después** (`[más adelante]`); el primario va abajo, en la zona del pulgar. **Nada del perfil se escribe hasta terminar**: lo que llevas se guarda al teclear (si la app se cierra, vuelve a su paso) y lo que dejaste para después no se escribe. Quien ya tiene perfil nunca lo ve. **Pendientes:** la cuenta (correo y código) entra como sus primeros pasos con v278 y el plan de pago con v279 | "pantalla por pantalla: usuario → biométricos → objetivo → split (ahora o después) → dieta (ahora o después) → Atajo de Salud → plan de pago", con `[‹ atrás]`; el formulario de una pantalla (v268 · v269) seguía "todo goofy" |
| 2026-09-24 | **Tour por sección** (v278): una cuenta nueva, **la primera vez que entra a cada sección** (gym sin split y con split, entreno con ejercicios, macros, progress y ajustes), ve de 3 a 5 pasos sobre **los controles reales**: un velo con hueco deja tocar el control (usarlo cuenta como siguiente) y un globo de vidrio (chrome, como las hojas) dice `// 1/4 · gym`, una línea en español y `[next]` `[skip tour]`. Nunca sale encima del arranque, de una hoja, de un aviso ni mientras escribes. **Tocar el velo lo pausa** hasta la próxima vez que abras la app; **`[skip tour]` apaga todos** (con `[deshacer]`) y en ajustes `guías de cada sección ···· repetir ›` los vuelve a prender desde cero. **Él nunca lo ve**: toda base que ya existía, y cualquier respaldo que se importe, llega con todo visto. Se adelantó porque no depende de nada: **las cuentas pasan a v279 y Pro a v280**, que esperan sus pasos en Supabase y en Lemon Squeezy | su encargo del 23-sep (un tour por sección para cuentas nuevas, que se pueda saltar), diseñado en la sección F del plan aprobado |
...
**Abierta desde v276:** qué versión del panel de macros se queda por defecto (el carrusel las muestra mientras elige, §9;
en el estudio, propuestas 25 progreso y 26 distribución, donde también está la dona).
**Abiertas desde v277 (choques del alta paso a paso con esta hoja):** `ready▌` usa el cursor fuera del arranque y los vacíos
(§3); el resumen final puede llevar 4 `✓` en verde sobre el pliegue (§8 pide ≤3; B-07: si está en orden, no lleva color);
y el alta mezcla `//SETUP` y `ready` en inglés con `▶ seguir`, `▶ ir al gym` y `[más adelante]` en español (§3: verbos de
comando en inglés y un componente en un solo idioma).
**Abiertas desde v278 (choques del tour con esta hoja):** cada paso es una instrucción impresa (B-10), aunque salga una sola
vez y flote; el globo cambia `[next]` por `[listo]` en el último paso y su etiqueta mezcla `gym`, `macros` y `progress` con
`entreno` y `ajustes` (§3), y el mismo concepto se llama `tour` en `[skip tour]` y `guías` en el aviso y en ajustes; la fila
de ajustes termina en `›` sin abrir nada (B-06), como las de descanso y beep.
