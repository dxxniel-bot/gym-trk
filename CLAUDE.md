...
## Pending / next
- **★ NEXT BIG TASK — canonical per-100 food model (math correctness).** User-approved design (math VERIFIED in node, all 5 label types incl. "envase total"). Goal: store every food canonically so logging math is always right regardless of label format (per-serving / per-100 / per-package / per-piece).
  ...
  - **Log = MyFitnessPal style:** unit dropdown built from `sizes` → `porción (30 g)` / `pieza` / `envase (240 g)` / `100 g` / `g`(or ml) + a qty field; grams = qty × unitGrams; nutrients = per100 × grams/100. Verified helpers: `gramsForUnit(food,unitKey)`, `foodUnits(food)`→[{key,label,grams}], `computeNutrients(food,unitKey,qty)`, `normalizeExtract(d)`→{base,per100,sizes,servingLabel} (converts whatever basis the label printed into per100 using serving_g / container_g; sets warnGrams when grams unknown).
