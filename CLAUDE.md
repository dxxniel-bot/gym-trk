...
- **Bump `sw.js` cache** (`gymtrk-vN`) on every shippable change so devices pick up the new build. Currently at **v46**.
...
- **Live barcode scanner** (`openBarcode()` → `startScan()`) — MFP-style: tap scan → camera opens immediately with center reticle overlay → auto-detects → found→`openVerify` (forward), not-found→`openRegisterMenu(code)` (register, barcode linked). **Two engines:** (1) native **`BarcodeDetector`** when available (Android Chrome/desktop) — own `<video>` + `getUserMedia` 1280×720 + `det.detect()` poll loop, smooth/HW-accelerated; (2) **Quagga2** fallback for iOS Safari (no BarcodeDetector) — **tuned for smoothness:** `facingMode:environment` ideal 1280×720, `patchSize:'medium'`, `halfSample:true`, `frequency:5`, `area` constrained to center band (matches reticle), readers EAN/EAN8/UPC/UPC_E, 2 consistent reads to accept. (The old 1080p/freq10/large-patch/no-halfSample config was janky on iPhone — "trabado" — hence the retune.) Manual code entry + 📷 photo-decode (`decodeBarcodeImage`, Quagga `decodeSingle`) are fallbacks. `window._scanner` holds `{_q,_fn}` (Quagga) or `{stream}` (native); `stopScan()`/`closeModal()` stop both (Quagga.stop + track.stop) so the camera light goes off on close. **Replaced html5-qrcode (v5 ZXing → v6 Quagga2)** because neither detected reliably on iPhone.
