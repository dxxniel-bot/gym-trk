# gym//TRK — DESIGN CHANGELOG

> **Histórico — nunca implementar desde aquí.** La identidad vigente es `BRAND.md` y el estado actual es
> `DESIGN_SYSTEM.md`. Este archivo guarda, **textual**, lo que salió de la guía cuando se separó en tres documentos
...
| 2026-09-23 | v267 | "terminal sobrio" (decisiones del dueño del 23-sep): esquinas 12/16/12 → control y tarjeta 4, flotante 8 (B-05 reescrita otra vez) · escala 10·12·18·24·34 → 10·12·14·20·28 + `--t-field` 16 solo en lo editable · campos con relleno `--card` y borde `--border` → caja fina transparente de 1 px `--o40` (foco = borde `--fg`) · primario 48/700 → 44 (`--h-pri`)/800, presionado = invertir · secundarios en caja → `[verbo]` con corchetes por CSS (B-06) y `[‹ back]` de texto · toggles en caja rellena → `[opción]` en `--fg`/700 · FS a 28 px con celda invertida · nav de íconos que se ensanchaba → texto `progress gym macros` con `>` que parpadea en la activa, sin animar layout, oculta también en histedit · glifos de la interfaz desde JetBrains Mono (`&text=`; ✓ ○ ↩ ⠿ no existen en la fuente) · `_dsRenderCheck` cuenta `rad` · cuenta nueva sin split: `[+ log past session]` abre una sesión libre en vez de fallar |
| 2026-09-23 | v268 | trabajo en curso como en Claude Code (el dueño, 23-sep): anillos giratorios de CSS (`.spin`, `.fa-spin`, `@keyframes spin`) → TRKSpin `▖▘▝▗ verbo… 3s` con un solo ticker de JS (120 ms, solo `textContent`, se apaga sin `[data-spin]`; reduced-motion = cuadro quieto, los segundos siguen) y TRKProgress `[██████▍░░░] 42%` con octavos (`role=progressbar`) en búsqueda en línea, código de barras, búsqueda por nombre, OCR (spinner y luego % real), IA de etiqueta, //ESPACIO y el toast de tarea (termina en ✓/⚠); `GLYPHS_VIZ` nuevo y aceptado por `_dsRenderCheck`; EXEMPT 31→28, MOTION 10→7 · arranque `loading gym tracker` en cada apertura (antes solo con usuario y ≥1 sesión, 1.3 s, `[ready]` parpadeando y ▲▼ de color): cabecera con `APP_V`, líneas reales una a una por `visibility` (db · split · last session), barra `[██████] 100%` y `ready▌` con el cursor en `loop`; ~950 ms, corto ~480 ms sin shader (sesión viva `> resuming <día> · set n/N` o abierta hace <30 min), reduced-motion todo a la vez; tocar lo salta; shader a `--op-dim` · Z-2 cerrado: la cola del arranque espera mientras exista `#bootov` · crear cuenta: formulario de campos gordos con etiquetas en MAYÚSCULAS → filas de terminal `.obr` (clave en minúsculas `--o50`, campo `.obi` de 40, `>` en la fila con foco, Enter al siguiente, pistas de una línea, vista previa `2,170 kcal · 122 g proteína al día` con `~` mientras son valores de ejemplo, `▶ empezar`); `onboardgo` aditivo (`since`, meta de sueño, `goalHist[hoy]`, peso tecleado = primer registro) · foco de campo sin anillo doble (`outline:none`) · `_v268SelfCheck` |
| 2026-09-24 | v269 | encargos del dueño del 24-sep (citas en BRAND §9) · **fuera el ánimo** ("Te había dicho de que quitaras lo del mood"): CSS `.moodpad`/`.moodpt`/`.moodax`, `MOOD_RING`/`moodVal`/`moodLabel`/`openMoodLog`, `logmood`/`editmood`/`delmood`, la línea `mood · today` de //STATS, la tile y su config, `metricSeries('mood')`, `metricBetter`, `READY_W.mood` (recovery = sueño .35 · FC .25 · HRV .20 · carga .25 · comida .15), `READY_ORDER` y `openReadiness`; higiene de datos 2: `purgeMood()` en `migrate()` (foto `pre-data2` en IndexedDB si había ánimo, `_dataVer` 2, `_dataLog.v2`, y `mood` se borra en cada migrate, respaldos viejos incluidos); `seed()` sin `mood` · **rest ≠ skip**: `rest` registra `{type:'rest'}` sin `dayId` y **ya no avanza `rotIdx`** (antes los dos avanzaban), con `[deshacer]`; `unrest`/`[quitar descanso]`; gym dice `hoy: descanso ✓`; `skip` avanza con `[deshacer]`; un día de solo descanso en el calendario abre "descanso programado · no movió tu split"; el recap dice `descanso` solo si se registró · **panel del anillo**: `.scroll` sin barra de scroll (el ancho ya no brinca al abrirlo), toda `.rule` dentro de `.kpanel` a sangre con `--s5` arriba y abajo, `[ver gramos]`/`[ver %]` baja de encima del radar a debajo de los anillos (`.kp-unit`), radar `--s4` abajo · **invitación de //SUPPS** en una cuenta sin stack: `[+ supp]` y `···` → `ignorar por ahora` (`db.settings.suppHide`, toast con `[deshacer]`), fila `supps en macros ···· ocultos · mostrar ›` en //SETTINGS · **cámara**: el emoji de cámara de fotos (`.camon::before`) → `CAM_SVG`, cámara de video de trazo 1.4 con punto de grabación en `--bad`, en `.camic` de 16 (R-GLYE 4→2) · **//PROGRESS editable como widgets**: la hoja on/off (`openProgConfig` con `progmtog` en `localStorage`) → `[edit]` o mantener 0.5 s una tile; `−` (`.pdel`), `⠿` (`.pgrip`, arrastrar o menú mover antes/después/quitar), `.pedbar` `[+ add] [cancel] ✓ listo`; `db.settings.progLayout {v:1, order, hidden}` (se hereda una vez de `gymtrk_progress_metrics_config`, viaja en respaldos), `PROG_KEYS`/`PROG_LBL`, `tdee` como tile propia; `openProgConfig()` queda como entrada al modo · **unidades**: `db.settings.bwUnit` (peso corporal, kg por defecto) aparte de `db.settings.unit` (pesas); `bwU`/`bwShow`/`bwParse` (siempre en kg, 0.01) en registro de peso, tile, detalle, throwback, wrap y //PROFILE (+ filas `peso corporal en ›` y `pesas del gym en ›`) · **perfil**: `[‹ atrás]`, unidades primero (`te pesas en`, `pesas gym`; el sufijo del peso sigue a la primera), `edad` y `peso` en filas propias, actividad y objetivo en listas verticales `.toggles.oblist` con su descripción fija (fuera las pistas que cambiaban abajo), `.obi` 44→36, clave 10.5ch→12.5ch · **`--t-field` 16→14 en toda la app** (el viewport ya lleva `maximum-scale=1`; la tabla de series lleva meses a 12 sin zoom) · `_v269SelfCheck`; `_readinessSelfCheck` con confianza 0.60 |
| 2026-09-24 | v270 | Atajo de Salud que pega **todo** ("se pegue lo del peso, … los pasos, … el HRV, … la frecuencia cardíaca en reposo, o sea, literalmente todo"): formato `trk2` línea por línea (`parseHealthPaste`), peso y grasa entran por `ingestHealth` con lo tecleado ganando, pegar en 2 toques (`healthPasteNow`), receta de 7 pasos (`openShortcutSetup`, `.hpst`/`.hpfmt`) · //STATS gana `health · paste` y sus renglones tocables pasan a ser el renglón entero (`.line.stat.tap`, mismo alto; hit 714 → 715 por el renglón nuevo; el +1 de `sheet:streak` contra la línea base de v269 es la fecha) · //HEALTH `[pegar de Salud] [cómo armar el Atajo] [importar JSON]` (se va `atajo (pasos/peso/sueño)`) · se adelantó al alta paso a paso para que su pantalla "conecta Salud" ya funcione |
| 2026-09-24 | v271 | encargos del dueño del 24-sep (citas en BRAND §9) · **cancel no loguea** ("a pesar de que le doy cancelar se me agrega a la meal ese alimento que no quería agregar"): en `openLog` el secundario `cancel` llevaba `data-act="logonly"` → `commitLog(false)` y registraba el alimento → `[cancel]` = `faback` (vuelve a la búsqueda con su texto) o `closemodal`, nunca registra; un alimento nuevo trae la fila `guardar en mis alimentos [sí] no` (`#l_save`, `optpick`) y el primario es siempre `loguear` (`logsave` lee la fila; antes `guardar + loguear` y un secundario `loguear sin guardar`, también `logonly`) · **`[+ food]` en la cabecera** ("el add food se pierde mucho… yo le añadiría los corchetes y lo pondría igual del lado derecho de la meal"): fila `.mit.add` al final de la comida abierta → `button.b.madd` entre el nombre y el total, 44 de alto y a la vista plegada; `.mhd` pasa a 4 columnas (nombre · `[+ food]` · kcal · `···`, `.gmore` en la 4) · **jerarquía** ("el nombre de la meal y el total de calorías… destaca más… las calorías totales que es el nombre de la meal"): `.mkc` `--fg`/800 → `--o70`/700; `.mnm` sigue `--t-section`/800 `--fg` · **macros por alimento de vuelta** ("le hace falta lo que antes tenía de que por alimento poner sus macros y en highlight como que su fuente principal"; elección: "Blanco en negrita, sin color"): `macroLineHTML()` → segunda línea `.mmac` `P 31 · C 2 · F 1` a `--t-label` `--o50` con la fuente principal por kcal en `<b>` `--fg`/700, `~` si es aproximado, también en `mealFoodRows()`; `mealDetailInline()` queda con la marca y `[editar] [mover] [borrar]` · **scroll** ("cuando le doy en ver gramos o ver porcentaje, se me scrollea hasta arriba… eso está muy molesto"): `togglemacros`/`toggleMacroUnit`/`ringtog`/`viewmeal`/`togglepreview`/`togglebeep` de `render()` a `reRender()`; `[ver gramos \| ver %]` de enlace de 10 (`u-label u-o50`) a `button.b` de 44; en modo % tocar un anillo da el % restante; R-SCROLL aprende a marcar interruptores con `render()` pelado (3 en v270 → 0) · **duraciones** ("hoy dormí 5.2 horas, pero ese 0.2 horas no me dice nada. así que hay que manejar horas y minutos, hay que manejar las unidades correspondientes de las cosas"): `5.2 h`, `1h26`, `6.2h (−1.8)` → `fmtDur()` `45 min` · `5 h 12 min` · `2 d 4 h` (con `fmtH`, alias `fmtHM`, `durHTML` para tiles, `sleepMinOf` al minuto y `fmtSleep`) en ~30 sitios (//STATS, tile `prom 7 h · 93% de la meta`, detalle con `tfmt` en horas redondas y `valHTML` con unidad chica, preparación, `dayFlags`, recuperación `hace 2 d 4 h`, cronómetro, agenda, recap, wrap, export `.md`, toast de Salud, FC del entreno, compartir sesión); `lineChart` gana `opt.tfmt`; el descanso sigue `m:ss` · `openShortcutSetup`: `instalar atajo` de segundo primario a `[verbo]` (R-OK vuelve a 6) · `_v271SelfCheck` · `dsSweep` ua 0 · hit 715 → 713 (−1 `macros`, −1 `sheet:foodadd`) · txt 133 · fsOff 0 · blur 0 · glyph 67 · rad 0 |
| 2026-09-24 | v272 | σ en lugar de la T, fuera MEV/MRV y el estado de cada ejercicio (citas en BRAND §9; modelo en `contexto/tension-v2.md` §3 y §6; aprobación: "me parece bien") · **σ** ("el tema de las series efectivas y del mínimo volumen recuperable y esa madre y de la GRP, esa madre realmente no sirve" · "sí el volumen importa, pero no volumen basura" · "si entrenas a 50 repeticiones RIR cero… es más bien fatiga cardiovascular"): la T de v219 → `setSigma()` `{t: S, I, est, k, C}` con `SIG`, `kEfOf` (RIR 0 anotado = 0.5; corrección de series largas), `sigTau`/`sigG`/`sigGr` (sin 1RM: compuerta por reps), `refE1RMFor(…, histOnly)`, `LEG_COMPOUND`/`HEAVY_PRESS`, `sigSat` (satura desde 10 por sesión y músculo); `setTension`/`setFatigueCost` quedan como alias · en pantalla la T pasa a σ: `σ0.98` por serie (`tensTag`), `~σ 2.6` por ejercicio (`exTensChip`), historial, hoja de sesión, detalle de volumen, export y tile `σ · wk` (clave interna `tension`; `PROG_LBL` `estímulo σ`); `GLOSS` `t`/`lm`/`stim` reescritos sin MEV/MRV ni guía RP; σ entra a `GLYPHS` · **fuera MEV/MAV/MRV**: `MUSCLES[].lm`, `lm` de `MUSCLE_GROUPS`, `VOL_LANDMARKS`, `MUSCLE_ALIAS`, `canonM`, `landmarksForLabel`, `volStatus`; en su lugar `SIG_BANDS` (<4 · 4–10 · 10–20 zona objetivo · 20–30 · >30, Pelland 2026) y `sigBarHTML()` con marcas **neutras** `.mev` en 10 y 20 (la `.mrv` en `--warn` ya no la emite nadie) · **//STIMULUS**: σ de 7 días por músculo real (con su etiqueta), frase solo del hallazgo principal o su banda sin color, color **solo en el `⚠`** (`dxGlyph`, B-07); "mucho fallo" en 3+ músculos se dice una vez arriba (`mucho fallo en N músculos · X% de sus series a F o RIR 0`); fila `⚠ fatiga acumulada · … bajando · ¿una semana ligera?` → `openDeloadInfo()` (5–7 días, series −30 a −50 %, RIR +2, los mismos ejercicios; Coleman 2024); meta `σ · 7 d` · **//MUSCLES**: `σ X · N series · RIR`, la banda junto al nombre solo si <4 o ≥20, barra 10/20; detalle: VOLUME en σ (banda, vs 7 d, `zona objetivo 10–20 σ por semana · Pelland 2026`), STIMULUS (`al fallo o RIR 0`, `sesión más cargada`), FATIGUE = costo C de 72 h vs su promedio de 4 semanas, pie que define σ y costo · **diagnóstico**: fuera `mrv`/`mev`; nuevas `failure`, `heavyF`, `high`, `session`, `lightFar`, `r30`, `low`, `legRec`, `regress`/`plateau`; `far` dice que RIR 1–2 rinde ~90 % · **estado del ejercicio** ("cuantificar si se está estancando, si se está progresando, si se está retrocediendo… acumulando fatiga"): `exHistKey`, `e1rmQ`/`sessE1RM`, `exSeries` (unidad de la vez más reciente; mismo gym en máquina), `exStatus` (Theil–Sen de ln e1RM, ventana 4/8/12 → 16 sem hasta 6 sesiones en ≥4 semanas), `EXST`, `exStatusTxt`; `fatigueFlag()` (≥2 retrocediendo en ~10 días y carga +20 % o >25 % a F/RIR 0); `recoveryEstimate` toma la dosis de C (+25 % con ≥2 F en compuestos de pierna) · **entreno**: el contador `N✓ N↑ N↓` → `obj RIR X` por peso relativo (`rirTargetTxt`: <55 % RIR 0–1 · 55–70 RIR 1–2 · 70–85 RIR 2 · ≥85 RIR 2–3), visible en modo enfoque en el ejercicio en curso; los números de serie ya no llevan color de zona · **e1RM**: `openLiftDetail`/`liftE1Series` en la unidad real (lbs/placas, ya no "kg" sobre libras), sin drops ni sugerencias, con línea `estado` · `_tensionSelfCheck` reescrito como tabla de calibración (`sigma self-check OK`), nuevo `_exStatusSelfCheck`, `_diagSelfCheck`/`_muscleSelfCheck` sin MEV/MRV · `dsSweep` ua 0 · hit 713 · txt 133 · fsOff 0 · blur 0 · glyph 67 → 72 (+5: la etiqueta del dueño `puh🥀` ahora sale en una fila de diagnóstico de //MUSCLES, en `progress` y 4 hojas encima) · rad 0 |
| 2026-09-24 | v273 | split: cómo entrenas, días sin gym, ciclo real y RIR o RPE (citas en BRAND §9) · **dato** (aditivo; sin plan = diario, igual que antes): `db.split.plan` `{mode: 'daily'/'weekly'/'cycle', on, off, week: {getDay: dayId}, blocked: [getDay]}`, `db.split.metric` (`rir`/`rpe`), `db.split.allowF` (`false` quita la F); las sesiones congelan `metric` (`newWorkSession`, `saveSession`, `finalizeIdleSession`; solo se escribe `'rpe'`, sin campo = RIR); las plantillas conservan plan, métrica y F · **motor** ("mi split actual realmente sería un split de ocho días, pues que lo calcule bien"): `splitPlan`/`splitMetric`/`splitAllowF`, `DOW_ES`/`DOW_LONG`/`DOW_INI`/`DOW_ORDER` (lunes primero), `trainedDates`, `planDay(iso)` → `{rest, why: 'blocked'/'plan'}` o `{idx}` — rotativo **adaptativo** (su regla: "si no se entrena = descanso"): tras `on` días entrenados seguidos toca descanso, un día sin entrenar ES descanso y no pide otro, un día bloqueado es descanso —, `cycleLen` (rotativo N + off × ⌈N/on⌉: 6 días a 3/1 = 8; días fijos 7; diario N), `planSummary`, `planForecast(n)`, `nextDateForDay`, `daysUntil`, `defaultWeek`; en días fijos `curDayIdx` = el próximo día de la semana con rutina · **editor**: //SCHEDULE antes de //COVERAGE y los días (`scheduleBlockHTML`: filas `.line.sch` de 44 con toggles `modo [diario] días fijos rotativo`, `días on 1–6`, `días off 1–3`; en días fijos un renglón por día con `[día]` → TRKMenu `descanso` o un día del split, y `sin gym` en los bloqueados; `días sin gym` con `L M X J V S D` en su propia línea (`.toggles.dow`); resumen `ciclo real · 8 días (6 de entreno + 2 de descanso) · sin gym: dom · si un día no entrenas, cuenta como descanso`; `intensidad [RIR] RPE`; `fallo (F) [sí] no`; handlers `sp_mode`/`sp_on`/`sp_off`/`sp_block`/`sp_wday`/`sp_metric`/`sp_allowf`) · //COVERAGE de `sets/vuelta` a `sets/sem` con el ciclo real (`11.4 sets/sem` · `cobertura planeada con tu ciclo de 8 días`) · **gym**: en un descanso del plan o un día sin gym, `.restplan` `hoy toca descanso · jueves sin gym` (o `tu plan 3 on / 1 off`) `· siguiente: mañana · <día>` + `[entrenar igual]`, sin primario; ROTATION `day 4/6 · 8 d` (rotativo) o `· lun` (días fijos); `+Nd` y la fecha del `.ics` con la fecha real (`nextDateForDay`); sin `[skip day]` en días fijos; toast del descanso `✓ descanso · mañana: <día>` → `✓ descanso · sigue <día> · mañana` · **racha**: los descansos del plan y los días sin gym cuentan (bit 4 en `activityDays`); un día de entreno que faltó la sigue rompiendo · **RPE** ("mi amigo utiliza el RPE"): se guarda como RIR (`rpeToRir`, 10 − RPE) y se muestra en la escala de la sesión (`rirShow`); la celda de RIR lleva `data-m`/`data-nf`; `openRirSelect` en RPE ofrece `F 10 9.5 9 8.5 8 7.5 7 6 5` en 2 filas de 44 (`trkSelect` con `o.wrap` → `.tselr.wrap`), sin F si `allowF` es `false`; cabecera de la tabla `rir`/`rpe` (`_icM`) y `GLOSS.rpe`; compartir, historial y export en la escala de la sesión (`_shM`: `@9`, `RPE 9`, ` · RPE9`); la etiqueta `@ RIR · F` se calcula (`intensityTag`, en la cabecera del día y el `.ics`) en vez de leer `day.tag` · `_scheduleSelfCheck` (33 self-checks en `?selftest=1`) · `dsSweep` sin cambios: ua 0 · hit 713 · txt 133 · fsOff 0 · blur 0 · glyph 72 · rad 0 (en el editor, los toggles nuevos miden ≥44 y ningún modo suma toques chicos) |
| 2026-09-24 | v274 | historial por ejercicio (cita en BRAND §9: "irme a progreso, seleccionar el ejercicio y… ver… un enlistado de… la fecha de la sesión… peso, número de repeticiones e intensidad, y… una gráfica de progreso respecto a ese ejercicio… como… las notas" de su amigo) · **clave** `exHistKey` (v272: nombre + variante, filas viejas sin tipo plegadas), **nunca `exId`** (cambia por día del split): sus calf rises de 3 días del split son una historia; `exKeyByName(name, hint)` (la variante con más sesiones; con pista, la de su lateralidad), `exHistRows(key)` (la más nueva primero), `openExHist(key)` (cierra el sheet, recuerda `state._exFrom`, `go('exhist')`) · **pantalla nueva `exhist`** (`renderExHist()`, en el router; sin nav, como history): `statusBar` + `[‹ back]` (`button.b`, `exback` → `state._exFrom` o progress) · `dayHeadHTML('EXERCISE', '[bi] nombre', 'machine · 17 sesiones · kg')` · línea de estado de v272 (`exStatusTxt()`: `progresando · +0.6 %/sem · 6 sesiones en 5 sem`) · TRKTabs `e1RM · peso top · volumen` (`extab`, `state._exTab`) · `lineChart` de detalle (150, etiquetas, promedio) con un punto por sesión en la unidad real (`sessE1RM()` · `effW()` mayor · Σ peso × reps; sin drops, sin `_pf`, sin series en otra unidad; en máquina/polea/smith solo el gym de la última sesión, regla v217, con la nota `la línea: solo AGON GYM · otra máquina no se compara`; con 1 punto `1 sesión en este rango: hacen falta 2 para la línea`) · periodos `30D 90D 6M 1A todo` (`exrange`, `state._exDays`, 6M por defecto) · //LOG `17 sesiones · la más nueva arriba` con filas `.elog`: `.eloh` = `.elon` `#17` (`--fg`/800) · `.elod` `23 sep · delts/push B · AGON GYM` (`--o60`, elipsis) · ▲▼ de capacidad (`progBadgeFromProg(histProgress())`), y `.elos` = las series con `shareExLines()` unidas por ` / ` en la escala RIR/RPE de cada sesión (`_shM`) + `· kg`; tocar una abre la sesión (`vieweditsession`) · **entradas**: //EXERCISES nueva en Progreso (con `cfg.e1rm`, después de //RECORDS; `exListHTML()`: `cada sesión de cada ejercicio`, sub `los de los últimos 60 días · veces · última`, filas `.line.exl` de 44 `[bi] nombre ···· progresando · 17 · 23 sep` con el nombre en elipsis y sin estado si es `pocos datos`, `[ver todos · N]`/`[solo los recientes]` con `exall`/`state._exAll`) · //RECORDS tocable (`.line.exr`, 44) · tile de e1RM y filas de //STRENGTH (`liftdetail`) con `data-k` → `exhist` (`openLiftDetail()` queda solo de respaldo) · catálogo `···` → `historial` (antes `historial · e1RM`) · perfil del ejercicio `[historial ›]` · entreno: tocar el nombre abre TRKMenu `historial` · `cambiar ejercicio` (sin historia, cambiar directo) · historial: el nombre de cada ejercicio (`.sxn`, `u-tap u-ul`) → `exhist` · `_exHistSelfCheck` (34 self-checks en `?selftest=1`) · `dsSweep` (ahora con `exhist`) ua 0 · hit 713 → 715 (solo los 2 de la barra de estado de la pantalla nueva: las filas de //EXERCISES y //RECORDS y las pestañas de la bitácora miden 44; los nombres bajan de renglón, no se cortan) · txt 133 · fsOff 0 · blur 0 · glyph 72 → 88 (+1 el `’` de un gym suyo en la bitácora; +3 en `progress` y en cada una de las 4 hojas que el barrido abre encima: la etiqueta del dueño `puh🥀` ahora también sale en //EXERCISES) · rad 0; `exhist` todavía no está en el recorrido |
| 2026-09-24 | v275 | suplementos con marca, frasco y aviso (citas en BRAND §9: "cuántas pastillas… por porción", "cuántas… trae el frasco", "una semana antes de que se acabe", "no lo encontré") · **dato** (aditivo y perezoso, nada se reescribe): el item de `db.stack` sigue siendo el genérico (mismo `id`: sus tomas, su historial y "tomar X" en //PERFORMANCE no se parten); nuevos opcionales `status` (`active`/`paused`/`archived`; `archived` se sigue escribiendo como `status ≠ active`, así los 8 lectores de `!x.archived` no cambian y un pausado también lo lleva), `statusLog` `[{d, to, why}]`, `products` `[{id, brand, name, form, per, size, dose, unit, nutr?, added}]`, `cur`, `containers` `[{id, prod, opened, size, adj?, closed?}]`; `SUPP_FORMS` (10 presentaciones con su unidad corta), `SUPP_WARN_D` = 7 · **motor**: `suppStatus`, `suppProd`, `suppCont`, `suppPU`, `suppSetStatus` (`se acabó` cierra el frasco; pausa y `no lo encontré` no), `suppOpenContainer` (cierra el anterior el día previo), `suppReactivate` (abre un frasco si no hay y el producto tiene tamaño), `suppLeft` (**lo que queda se calcula**: lo que trae − días con toma tomada o tarde desde que se abrió × por toma + `adj`; la saltada no gasta; los 4 caminos de tomas y el deshacer no lo desfasan), `suppDaysLeft` (tomas que quedan ÷ tomas por día de su periodización en 28 días), `suppStock` `{L, d, out, low}`, `suppStockTxt` (`quedan 10 softgels · ~5 d` / `se acabó`), `suppCellMark`, `suppProdOn`/`suppNutrOn` (la nutrición de un día sale del producto abierto ese día; `macroTotals()` la usa, solo con lo marcado), `suppFindDormant`, `suppSnap`/`suppRestore` (deshacer exacto), `suppWarnMaybe` · **stack**: la marca tras el nombre y, bajo la periodización, `quedan 80 cáps · ~40 d` (`--o40`) o `⚠ quedan 10 softgels · ~5 d` / `⚠ se acabó` (`--fg`, el `⚠` en `--warn`); en TODOS, bloques `EN PAUSA · N` (abierto) y `ARCHIVADOS · N` (cerrado): nombre (tocar = editar) · marca · motivo · fecha ···· `[reactivar]` (`suppreact`) · **//SUPPS**: la celda suma `⚠ ~5 d` / `⚠ se acabó` (`--t-label` `--o40`); una vez al día al pintar macros (`db.settings.suppWarnDay`) toast `⚠ omega-3 · quedan 10 softgels · ~5 d` o `⚠ N suplementos por acabarse · a, b, c` con `[ver]` → stack en TODOS; `toast()` gana `opt.act {label, fn}` y un toast con acción se va solo a `--toast-life-err` · **editor** (`openStackEdit(id, presetCat, opt)`): bloque `PRODUCTO · FRASCO · opcional · para avisarte antes de que se acabe` tras DOSIS — `MARCA` (datalist de sus marcas) | `PRODUCTO`, `PRESENTACIÓN` | `POR TOMA · <unidad>`, `TRAE EL FRASCO` | `LO ABRISTE`, `QUEDAN HOY` (corrección → `adj`), línea de stock + `[abrí otro frasco]` y, si se acabó, `[se acabó · archivar]` `[pausa]`; el pie `borrar` en rojo → `más` → TRKMenu: activo `pausar` / `archivar · se acabó` / `archivar · no lo encontré` / `borrar · con su historial` (TRKHold, lo último); en pausa o archivado `reactivar` / `borrar`; cada cambio de estado con `[deshacer]` exacto · **volver a agregarlo**: guardar uno nuevo con el nombre de uno dormido abre TRKMenu `"omega-3" ya estaba archivado · Nordic`: `volver con Nordic` / `volver con otra marca` (su editor con `opt.newBrand`: marca vacía, historial intacto) / `crear otro aparte` (`opt.forceNew`); marca nueva = producto nuevo + frasco nuevo, el anterior guarda su dosis y nutrición para los días viejos; si cambió presentación, por toma o dosis, TRKAsk `cambió con la marca nueva` (`por toma: 2 softgels → 1 softgels`, `dosis: 2 g → 1 g`) con `[así queda]` `[revisar]` · **arreglos**: la invitación de //SUPPS cuenta solo los activos (todo archivado → vuelve); un suplemento agregado viendo un día anterior en macros empieza ese día · export //REGÍMENES con marca, por toma y lo que queda · `_suppSelfCheck` (35 self-checks en `?selftest=1`) · `dsSweep` sin cambios (ua 0 · hit 715 · txt 133 · fsOff 0 · blur 0 · glyph 88 · rad 0: su stack aún no tiene productos); medido con productos: stack hit 43 → 41, editor 41 → 40, ningún toque chico nuevo |
| 2026-09-24 | v276 | laboratorio de macros (encargo en BRAND §9: propuestas de barras de progreso y de distribución de los macros, verlas "en un carrusel" dentro del panel mientras elige, y una versión para compartir) · **motor**: `MACRO_VIZ` = `[clave, pestaña, solo laboratorio]` (`rings` aros · `bars` barras · `meter` medidor · `split` reparto · `table` tabla · `donut` dona, esta solo laboratorio: BRAND §4 deja al anillo de kcal como la gráfica circular y §8 dice no a los pasteles), `macroVizKey()` (`db.settings.macroViz`, inválida → aros), `macroVizList()` (las 5 + la elegida), `macroSplit(t)` (parte de las kcal de P·4 C·4 F·9 en tres enteros que suman 100), `macroVizHTML(k, t, g, o)` (una sola función para el carrusel, la tarjeta y el laboratorio del estudio: propuestas 25 `macroprog` y 26 `macrodist`, donde se elige la dona): aros = el radar (`dayRadar`) + los 3 anillos de antes · barras = filas `.mrow` con `142 / 180 g` (o % con `[ver %]`), >105 % en `--bad` · medidor = TRKProgress de 12 celdas + `153/150 g` · reparto = barra apilada `.mvstack` de hoy sobre una fina de la meta, en escala de opacidad (`.mvp` `--fill` · `.mvc` `--o60` · `.mvf` `--o30`) + leyenda `P 28% · meta 25` + `% de las kcal · P·4 C·4 F·9` · tabla = `.mvtab` hoy / meta / % para kcal, protein, carbs y fat (>105 % solo el número en `--bad`) · dona = arcos SVG (trazo 1.8) + leyenda · **carrusel** (`mvzHTML`, en el panel abierto en lugar del radar + anillos fijos): `.mvtrack` con `scroll-snap` horizontal (una `.mvslide` por versión, `align-items:flex-start`, `overflow-y:hidden`), pestañas de texto **abajo** (`.mdtabs.tall.mvtabs`, 44 de alto, mínimo 44 de ancho; `mvtab` → `scrollTo` suave, instantáneo con reduced-motion) y luego el `[ver gramos \| ver %]` de siempre; `mvzSync()` tras cada render (hook en `render()` para macros) pone el carril en la versión guardada sin animar y le da el alto de la lámina a la vista (la más alta dejaba huecos enormes); un listener de scroll con retardo guarda `db.settings.macroViz` y cambia las pestañas sin repintar · **compartir**: `[share]` en macros abre TRKMenu `compartir` → `el día · tus comidas` (la tarjeta de siempre) / `el panel de macros` (`state.shareType` `'macros'` → `renderShareMacros()`: anillo de kcal grande `.shmhero` en verde/rojo como en la app + la versión elegida, vertical a su altura natural, marca gym//TRK por `summaryShell`; `macroShareText()` para copiar texto); `shareImage()` aprendió a pintar barras (`.bar`, `.bar>i`, `.mvstack`, `.mvstack>i`, `.mvleg i`) y SVG simple (radar y dona: `polygon`, `line`, `path`, `circle`, `text`) · `_macroVizSelfCheck` (36 self-checks en `?selftest=1`) · `dsSweep` sin cambios (el panel se mide cerrado); medido abierto con sus datos, los mismos toques chicos que en v275 (25) en las 5 versiones del carrusel · `ds-audit --strict` en verde; línea base `tools/ds-baseline.json` rotulada v276 |
| 2026-09-24 | v277 | alta paso a paso (encargo en BRAND §9: "pantalla por pantalla: usuario → biométricos → objetivo → split (ahora o después) → dieta (ahora o después) → Atajo de Salud → plan de pago", con `[‹ atrás]`) · **forma**: el formulario de una pantalla de v268 · v269 (`gym//TRK//PROFILE`, diez filas y `▶ empezar`) → 10 pasos, una pregunta por pantalla (`ONB_STEPS`): nombre y gym · unidades (peso corporal y pesas del gym, **antes** del cuerpo) · cuerpo (sexo, edad, estatura y peso en tu unidad + `mantenimiento 2,610 kcal al día` en vivo, con `~` mientras sean los de ejemplo) · actividad (`OB_ACT`) · objetivo (`OB_GOAL` con sus kcal) · cómo entrenas (el plan de v273: modo, días on 1–6 / off 1–3, días sin gym, RIR o RPE con una línea que lo explica) · rutina (`SPLIT_TEMPLATES` + `desde cero`) · metas del día (kcal, proteína, carbos, grasa y agua, calculadas y editables) · Salud (iPhone: el Atajo con `[cómo se arma el Atajo]` → `openShortcutSetup`; Android: a mano) · listo (resumen `✓`/`—` + `ready▌` + `▶ ir al gym`); los 4 opcionales traen `[más adelante]` · **chrome**: `[‹ atrás]` siempre (en el 1 vuelve a la portada), `//SETUP 3/10`, barra `[███░░░░░░░]` (`.onbbar`: hechas en `--fg`, el resto en `--o40`, `aria-hidden`), la pregunta `.onbq` (`--t-display`/800) + su porqué (`.submeta`), las filas de v268 · v269 (`.obr`/`.obk`/`.obi` de 36 en fila de 44, `.oblist`) y la nueva `.obr.obstack` (clave arriba, `.toggles.dow` a todo lo ancho), error en línea `.onberr` (`role="alert"`, `⚠` en `--warn`: `⚠ peso en lbs, entre 66 y 550`; antes un toast `⚠ pon tu nombre`) y el pie `.onbfoot` pegado abajo (zona del pulgar) · **dato**: `db.onb` = `{step, done, d}` guardado en cada toque y cada tecla (`saveSoon`; si la app se cierra, vuelve a su paso); **nada del perfil se escribe hasta el final**: `onbApply(d)` escribe perfil, gyms, `settings.unit`/`bwUnit`, metas (cálculo + lo tecleado), `goalHist[hoy]`, `bodyweight[hoy]` en kg (150 lbs = 68.04), el split de la plantilla (o `mi split` con un día vacío) con `plan` y `metric` (días fijos → `defaultWeek()`), `rotIdx` 0 y `onb` = `{done, at, skip}`; lo que dejaste para después no se escribe; `onbFinish` → gym (o el editor de split con `desde cero`) · `migrate()` marca `onb.done` a cualquier base con usuario: el dueño nunca lo ve · router: sin usuario, un alta a medias (`step` > 0) abre en su paso; `pushState` por paso y `popstate` = un paso atrás · validación `onbCheck` (nombre obligatorio; edad 13–99, estatura 120–230 cm y peso 30–250 kg revisado en tu unidad) · se van `obPick`, `obDraft`, `obPrev` y el handler `onboardgo` (su prueba de v268 pasa a `onbApply`) · bug de camino: un comentario `//` dentro de `migrate()` (una sola línea) se comía el resto de la función; lo cazó `_streakSelfCheck` y quedó como `/* */` · `_onbSelfCheck` (37 self-checks en `?selftest=1`) · `dsSweep` sin cambios (ua 0 · hit 715 · txt 133 · fsOff 0 · blur 0 · glyph 88 · rad 0; el alta no entra al barrido); `_dsRenderCheck` en cada paso: txt 0, hit solo las casillas de 36 (1–5 por paso) dentro de filas de 44 · `ds-audit --strict` en verde (P2: R-OK 6 → 7 por los dos primarios de un ternario, R-LH 12 → 13 por `.onbq`) · pendientes: la cuenta (correo y código) con v278 y el plan de pago con v279 |
| 2026-09-24 | v278 | tour por sección (encargo del 23-sep en BRAND §9, sección F del plan aprobado; se adelantó porque no depende de nada: **cuentas → v279 y Pro → v280**, que esperan sus pasos en Supabase y Lemon Squeezy) · **pasos** (`TOUR_STEPS` `{sel, t}` + `TOUR_LBL`, 3–5 por sección): gym sin split (`newsplit` · `workshop` · `importsplit` · `navmenu` · `#nav` "el > marca dónde estás") · gym con split (`start` · `.arrow` `nextday` `‹ ›` · `loglater` · `navmenu`) · entreno (el primer peso "escribe peso y reps; desde la segunda vez, lo gris es lo que hiciste la última vez" · `.rirb` RIR o RPE según la métrica de la sesión · `done` ✓ y descanso · `addset` `[+ set]`/`[↓ drop set]` · `#wfooter .save`) · macros (`togglemacros` el anillo · `newmeal` · `waterpick` · `dayprev`) · progress (`streakcal` · la primera `.ptile` · `progedit` `[edit]`) · ajustes (`pf_goals` · `export` "tus datos viven en este teléfono" · `tourreplay`) · **motor**: `tourKey` (la sección de la pantalla; entreno solo con ejercicios; nada sin usuario), `tourOn`, `tourBusy` (arranque `#bootov`, hoja `#modal`, `#asklayer` o una casilla con foco → reintenta a los 800), `tourMaybe` (al final de `afterPaint`, 350 después), `tourShow` (busca el ancla en cada render; un ancla que falta o no mide se salta; sin ninguna, la sección queda vista; centra el ancla de `#view` con `scrollIntoView` instantáneo), `tourDraw` (`#tour.tour` en `#app`, absoluto `inset:0`, `z-index:var(--z-tour)` = **45, token nuevo** entre `--z-modal` 40 y `--z-pop` 50: 4 rectángulos `.tm` de `--scrim` alrededor de un hueco de 6 sobre el **control real, que se sigue tocando**; anillo `.tr` de `--bw-field` en `--fg` y `--r-ctl`; globo `.tourb.glass-strong` a `--r-float` debajo del hueco si caben 150, si no arriba, con `// 1/4 · gym` (`.tourh` `--t-label` `--o50`), una línea (`.tourt` `--t-data` `--fg`, `--lh-read`) y `[next]`/`[listo]` `[skip tour]` (`button.b`); se redibuja con el scroll de `#view`), `tourAdvance` · usar el control de verdad = siguiente (click en captura sobre el ancla) · tocar el velo pausa la sección hasta la próxima apertura (`_tourPause`, solo en memoria) · `[skip tour]` → `db.tour.skipped` + toast `guías apagadas · se prenden en ajustes` con `[deshacer]` · ajustes: fila `.line.lnav` de 44 `guías de cada sección ···· repetir ›` (o `apagadas · prender ›`) → `tourreplay` pone `db.tour` en blanco y sale el tour de ajustes · **dato**: `db.tour` = `{v:1, seen:{}, step:{}, skipped:false}`; `migrate()` marca las 6 secciones vistas a cualquier base sin `tour` (él nunca lo ve, tampoco un respaldo importado); `onbApply()` lo crea en blanco para una cuenta nueva · **auditor**: `tools/ds-rules.cjs` suma `.tour`/`.tourb` a CHROME y `tourDraw` a las funciones que pueden poner vidrio en el marcado; `DS_CHROME` de `index.html` suma `.tour` · `_tourSelfCheck` (38 self-checks en `?selftest=1`) · `dsSweep` sin cambios (con su respaldo no sale; la fila nueva mide 44) · capturas de una cuenta nueva recién salida del alta: gym vacío 1/5, gym 1/4 y 2/4, macros 1/4, entreno 1/5, ajustes 3/3; con su respaldo no aparece · `style=""` 87 → 90 (las posiciones medidas de `tourDraw`) · `ds-audit --strict` en verde · choques anotados en BRAND §10 y DESIGN_SYSTEM §7.42: `[next]` → `[listo]`, etiquetas `entreno`/`ajustes` junto a `gym`/`macros`/`progress`, `tour` y `guías` para lo mismo, instrucción impresa (B-10), `›` en una fila que no navega, `--ls-title` en minúsculas, `--bw-field` en un indicador y el toast de repetir de 43 caracteres |

## Diseños rechazados por el dueño (no volver a proponer)

| fecha | qué | cita |
|---|---|---|
| 2026-09-21 | Tarjeta de compartir sesión con tres lecturas en `--t-hero` (duración · tonelaje · T) y rejilla (U7/E21, v252) | "ese diseño de share workout está del pito" |
| 2026-09-21 | Compartir en cuadro 1:1 que nunca crece y recorta (`fitShareCard`, v256) | "pésimo… todo feo, amontonado" |
| 2026-09-19 | Racha en gris + verde | "el gris y el verde se ven feos" |
| 2026-09-21 | Terminal pura sin vidrio (recomendación de la auditoría) | "que todo sea un mix cmdhacker/glass moderno" |

---

## 1. Estado y plan de corrección por fases (§20 de la guía anterior, textual)
...
## 2. Guía anterior completa (DESIGN_SYSTEM.md tal como estaba en v257, textual)

> Incluye las notas "vNNN ·", "Hecho …" y los registros DS-x / R-x / U-x que antes vivían dentro de cada sección.
