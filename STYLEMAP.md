...
- `.setend` (solo `.del` 18px centrado — v215 quitó el `↓` por fila; drop set solo vía barra inferior `[↓ drop set]`) · `.srow.isdrop`/`.pair.isdrop` (opacity .82 — solo visual, mismas columnas grid que fila normal) · `.gc-cardio` (grid cardio: set# + campos del subtipo + del).
